/**
 * Карта "название Unusual-эффекта -> числовой id частицы" — портировано
 * дословно из Steam Trade Offer Enhancer 2.2.8 (shared.offers.unusual.effectsMap),
 * единственного из трёх изученных проектов, где список эффектов полный
 * (обычные + war paint / island cosmetic эффекты). 541 запись.
 *
 * ID используется для URL картинки эффекта (см. getEffectURL в utils/icons.ts)
 * и для построения SKU (utils/item-attributes.ts).
 */
export const UNUSUAL_EFFECTS_MAP: Record<string, number> = {
  'Green Confetti': 6,
  'Purple Confetti': 7,
  'Haunted Ghosts': 8,
  'Green Energy': 9,
...
export function getEffectId(effectName: string): number | undefined {
  return UNUSUAL_EFFECTS_MAP[effectName];
}

/**
 * Обратная карта id -> название — лениво строится один раз при первом
 * обращении (реестр статический, 541 запись, ни один id не встречается
 * дважды — проверено скриптом при добавлении этой функции). Нужна
 * modules/trade-item-summary (режим с ценами PriceDB.io, utils/pricedb.ts):
 * чтобы получить цену конкретного Unusual-эффекта, sku.pricedb.io ожидает
 * на входе "<Effect Name> <Item Name>" (без слова "Unusual" — подтверждено
 * живым запросом: "Unusual Team Captain" резолвится в effect:null, а
 * "Burning Flames Team Captain" — в effect:13, sku "378;5;u13"), а у нас
 * есть только числовой id эффекта (getItemAttributes), не имя.
 */
let effectNameById: Map<number, string> | undefined;

export function getEffectName(effectId: number): string | undefined {
  if (!effectNameById) {
    effectNameById = new Map();
    for (const [name, id] of Object.entries(UNUSUAL_EFFECTS_MAP)) {
      effectNameById.set(id, name);
    }
  }
  return effectNameById.get(effectId);
}
